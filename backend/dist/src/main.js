...
import { prisma } from "./prisma.js"; // 👈 KROK 4: Taháme sdíleného klienta z jednoho místa
...
//ADMINSETUSER
async function adminsetuser(req, res, next) {
...
async function formatGradeResponse(grade) {
...
async function getUserGrades(req, res, next, userId) {
    try {
        /// AUTH ///
...
app.get('/api/events', async (req, res, next) => {
    ...
    const currentPrivilege = privileges[currentRole] ?? 0;
    ...
    const newEvent = await prisma.event.create({
        data: {
            ...
            participantsIndividuals: {
                connect: participantIndividualIds.map((id) => ({ id }))
            },
            participantsClasses: {
                connect: participantClassIds.map((id) => ({ id }))
            }
        },
        include: {
            participantsIndividuals: true,
...
app.put('/api/events/:id', async (req, res, next) => {
    ...
    const updatedEvent = await prisma.event.update({
        ...
        data: {
            ...
            allDay,
            participantsIndividuals: {
                set: participantIndividualIds.map((id) => ({ id }))
            },
            participantsClasses: {
                set: participantClassIds.map((id) => ({ id }))
            }
        },
        include: {
            participantsIndividuals: true,
